...
for  j=1:niter
    ...
    phi, g1 = lsrtm_objective(model0_stack[1], q_j[1], dobs_j[1], x[1]; options=opt, nlind=true)
    g = C*Mr'*vec(g1)
	...
	global t = 5f-7 # fixed step

    # anti-chatter
    for i = 1:1
        global sumsign[i] = sumsign[i] + sign.(g)
        tau = t*ones(Float32,size(C,1))
        tau[findall(flag[i])] = deepcopy((t*abs.(sumsign[i])/j)[findall(flag[i])])
        global z[i] -= tau .* g
    end

	(j==1) && global lambda = quantile(abs.(vec(z[1])), .8)	# estimate thresholding parameter at 1st iteration

	# Update variables and save snapshot
	for i = 1:1
		global x[i] = adjoint(C)*soft_thresholding(z[i], lambda)
	end
    JLD2.@save "../results/LSRTMsim$(j)Iter$(nsrc)nsrc.jld2" x z g lambda phi
end
