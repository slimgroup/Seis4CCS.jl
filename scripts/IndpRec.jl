...
for  j=1:niter
	...
	(j==1) && global lambda = [quantile(abs.(vec(z[i])), .9) for i = 1:L]	# estimate thresholding parameter at 1st iteration
    ...
    JLD2.@save "../results/IndpSimSrc$(j)Iter$(L)vintages$(nsrc)nsrc.jld2" x z g lambda phi
end
